...
if(WITH_POST_QUANTUM_CRYPTO AND NOT OPENSSL_FOUND)
    message(FATAL_ERROR "WITH_POST_QUANTUM_CRYPTO is only usable with the OpenSSL option")
endif(WITH_POST_QUANTUM_CRYPTO AND NOT OPENSSL_FOUND)

if(WITH_POST_QUANTUM_CRYPTO AND NOT OQS_ROOT_DIR)
    message(FATAL_ERROR "Need to supply installation path for OQS in OQS_ROOT_DIR")
endif(WITH_POST_QUANTUM_CRYPTO AND NOT OQS_ROOT_DIR)

set(OQS_LIB_DIR ${OQS_ROOT_DIR}/lib)
set(OQS_INCLUDE_DIR ${OQS_ROOT_DIR}/include)
...
if (OPENSSL_FOUND)
    ...
    if (WITH_POST_QUANTUM_CRYPTO)
        set(CMAKE_REQUIRED_INCLUDES ${OQS_INCLUDE_DIR} ${OPENSSL_INCLUDE_DIR})
        check_include_file(oqs/oqs.h HAVE_OQS_H)
        if (NOT HAVE_OQS_H)
            message(FATAL_ERROR "WITH_POST_QUANTUM_CRYPTO selected but could not detect oqs/oqs.h")
        endif(NOT HAVE_OQS_H)
        set(CMAKE_REQUIRED_LIBRARIES ${OQS_LIB_DIR}/liboqs.a ${OPENSSL_CRYPTO_LIBRARIES})
        check_function_exists(OQS_KEM_encaps HAVE_OQS_KEM_ENCAPS)
        if (NOT HAVE_OQS_KEM_ENCAPS)
            message(FATAL_ERROR "Could not locate OQS_KEM_encaps")
        endif(NOT HAVE_OQS_KEM_ENCAPS)
        set(CMAKE_REQUIRED_LIBRARIES ${OQS_LIB_DIR}/liboqs.a ${OPENSSL_CRYPTO_LIBRARIES})
        check_function_exists(OQS_KEM_decaps HAVE_OQS_KEM_DECAPS)
        if (NOT HAVE_OQS_KEM_DECAPS)
            message(FATAL_ERROR "Could not locate OQS_SIG_verify")
        endif(NOT HAVE_OQS_KEM_DECAPS)
        set(CMAKE_REQUIRED_LIBRARIES ${OQS_LIB_DIR}/liboqs.a ${OPENSSL_CRYPTO_LIBRARIES})
        check_function_exists(OQS_SIG_sign HAVE_OQS_SIG_SIGN)
        if (NOT HAVE_OQS_SIG_SIGN)
            message(FATAL_ERROR "Could not locate OQS_SIG_sign")
        endif(NOT HAVE_OQS_SIG_SIGN)
        set(CMAKE_REQUIRED_LIBRARIES ${OQS_LIB_DIR}/liboqs.a ${OPENSSL_CRYPTO_LIBRARIES})
        check_function_exists(OQS_SIG_verify HAVE_OQS_SIG_VERIFY)
        if (NOT HAVE_OQS_SIG_VERIFY)
            message(FATAL_ERROR "Could not locate OQS_SIG_verify")
        endif(NOT HAVE_OQS_SIG_VERIFY)
    endif()
